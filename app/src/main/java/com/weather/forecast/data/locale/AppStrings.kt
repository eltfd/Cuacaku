...
class AppStrings(val locale: AppLocale) {
    ...
    val seismicDataNote = s(
        "Data gempa dari BMKG (resmi Indonesia) & USGS (global). Laporan bencana dari PetaBencana.id (crowdsource). Data gunung api dari USGS & NASA EONET. Risiko longsor dari Open-Meteo (curah hujan & kelembaban tanah).",
        "Earthquake data from BMKG (official Indonesia) & USGS (global). Disaster reports from PetaBencana.id (crowdsource). Volcano data from USGS & NASA EONET. Landslide risk from Open-Meteo (rainfall & soil moisture)."
    )
    val waveDataNote = s(
